...
def plot_feature_importance(feature_importances, feature_names, method, data_name):
    plt.figure(figsize=(10, 6))
    colors = ['orange' if imp < 0 else 'lightcoral' for imp in feature_importances]
    plt.barh(range(len(feature_importances)), feature_importances, color=colors, align='center')
    plt.yticks(range(len(feature_importances)), feature_names)
    plt.xlabel(f'{method} Feature Importance')
    plt.title(f'{method} Feature Importances')

    path = f'results/{data_name}'
...
def mapping_result(arr1, arr2, arr3, arr4, arr5, arr6, labels):

    # assign data
    data = []

    headers = [" ", "LIME", "Integrated Gradient", "SHAP", "Smooth Gradient", "Input Times Gradient", "Gradient"]
