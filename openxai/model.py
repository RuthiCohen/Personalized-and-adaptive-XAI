...
def train_model(model_name, dataset, learning_rate, epochs, batch_size,
                scaler='minmax', seed=0, warmup=5, verbose=False):
    """
    ...
    n_class = len(set(trainloader.dataset.targets))

    # Define the model
    torch.manual_seed(seed)
    if model_name == 'ann':
        model = ArtificialNeuralNetwork(input_size, [100, 100], n_class=n_class)  # Update n_class
    else:
    ...
    # Adjust loss function for multi-class classification
    criterion = nn.CrossEntropyLoss()  # CrossEntropyLoss handles multi-class classification automatically
    ...
    # Training loop (same as before, with updated n_class)
    for e in range(epochs):
        for phase in ['train', 'test']:
            model.train() if phase == 'train' else model.eval()
            running_loss, running_acc, n_inputs = 0.0, 0.0, 0
            for inputs, labels in loaders[phase]:
                ...
                # Track statistics
                preds = y_pred.argmax(dim=1)  # Get the class prediction
                running_acc += (preds == labels).sum().item() / inputs.size(0)
            ...
            if verbose:
                print(f'Epoch {e}/{epochs - 1} | {phase} | Loss: {epoch_loss:.4f} | Acc: {epoch_acc:.4f}')
